...
void DCMotorTestCase::testCommit() {
	...
	CPPUNIT_ASSERT_MESSAGE("Port enabling graduality problem", testGraduality(analogPortWritings[2], 0, 1, 100));
	...
	CPPUNIT_ASSERT_MESSAGE("Current direction is: " + std::to_string(motor->getDirection()) + " expected: " + std::to_string(direction), (int) motor->getDirection() == (int) direction);
	CPPUNIT_ASSERT_MESSAGE("Port should go from 100 to 0", testGraduality(analogPortWritings[2], 0, 99, 0));
	CPPUNIT_ASSERT_MESSAGE("And after it is 0, it should go to 255", testGraduality(analogPortWritings[1], 99, 0, 255));
...
void DCMotorTestCase::testSmooth() {
	...
	motor->setSpeed(100);
	motor->adjustSpeed();
	CPPUNIT_ASSERT_MESSAGE("Port should go from 149 to 100", testGraduality(analogPortWritings[1], 0, 149, 100));
	CPPUNIT_ASSERT_MESSAGE("Both motor directions enabled at the same time", testNoParallel(analogPortWritings, 1, 2));
	CPPUNIT_ASSERT_MESSAGE("AnalogPort Contains: " + std::to_string(analogPortData[2]), analogPortData[2] == 0);
